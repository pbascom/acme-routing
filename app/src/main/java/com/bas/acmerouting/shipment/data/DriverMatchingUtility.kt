...
class DriverMatchingUtility {
    fun adapt(response: ShipmentResponse): Map<Driver, Route> {
        ...
        // suitability to take each route, using the following rules:

        // If the length of the shipment's destination street name is even, the base suitability
        // score is the number of vowels in the driver's name multiplied by 1.5

        // If the length of the shipment's destination street name is odd, the base suitability
        // score is the number of consonants in the driver's name multiplied by 1

        // If the length of the shipment's destination street name shares any common factors (other
        // than 1) with the length of the driver's name, the SS is increased by 50% above the base SS.

        // Next, we must assign drivers to routes which maximize the total suitability over the
        // entire dataset.
        ...
        val analyzedDestinations = response.routes.map { destination -> AnalyzedDestination.create(destination) }
        ...
        // and so on. This algorithm is vulnerable to getting stuck in local minima, but I'm an
        // Android engineer not a Graph Theorist.
        ...
        while(!suitabilityQueue.isEmpty()) {
            ...
            shipments[Driver(mostSuitableMatch.driver.name)] = Route(mostSuitableMatch.destination.destination)
        }
    ...
    data class AnalyzedDestination(
        ...
        companion object {
            fun create(routeDestination: String): AnalyzedDestination {
                return AnalyzedDestination(
                    streetNameLength = routeDestination.count { !it.isWhitespace() && !it.isDigit() },
